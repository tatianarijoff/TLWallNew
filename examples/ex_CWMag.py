"""
Example: Coated-wall with magnetic layer, compute impedances and export results.

Demonstrates:
- How to load a tlwall configuration from a .cfg file (coated-wall magnetic case)
...
def example_cwmag_from_cfg():
    """Example: Coated-wall chamber with magnetic layer from configuration file."""
    print("=" * 70)
    print("Example: Coated-Wall Magnetic Configuration from ex_CWMag.cfg")
    print("=" * 70)

    # ------------------------------------------------------------------
    # Read configuration and build TlWall object
    # ------------------------------------------------------------------
    cfg_path = os.path.join("examples/ex_CWMag", "ex_CWMag.cfg")
    ...
    # Save data to Excel
    # ------------------------------------------------------------------
    outdir_data = os.path.join("examples/ex_CWMag", "output")
    ...
    # Plot impedances
    # ------------------------------------------------------------------
    outdir_img = os.path.join("examples/ex_CWMag", "img")
    ...
    print(f"  |ZLong| min/max: {np.min(np.abs(wall.ZLong)):.3e} / "
          f"{np.max(np.abs(wall.ZLong)):.3e} Ohm")
    print(f"  |ZTrans| min/max: {np.min(np.abs(wall.ZTrans)):.3e} / "
          f"{np.max(np.abs(wall.ZTrans)):.3e} Ohm/m")
    print()


if __name__ == "__main__":
    example_cwmag_from_cfg()
    print("=" * 70)
    print("Coated-wall magnetic example completed successfully!")
    print("=" * 70)
